...
//颁发机构CA证书TLS 证书颁发机构 （CA） 文件。它用于验证 服务器发送的“公共”证书
...
//生成自签名 服务器证书和密钥文件:openssl req  -nodes -new -x509  -keyout key.pem -out cert.pem
//TLS 证书。这是一个“公共”部分。例如，启用 TLS 的服务器 在 TLS 握手期间将其发送到客户端
...
//生成自签名 服务器证书和密钥文件:openssl req  -nodes -new -x509  -keyout key.pem -out cert.pem
//TLS 私钥。这是一个“私人”部分
static const char* s_tls_key =//私钥
"-----BEGIN PRIVATE KEY-----\n"
...
int main(void) {
  ...
  struct mg_tls_opts opts = { .client_ca = mg_str(s_tls_ca),//颁发机构CA
                             .server_cert = mg_str(s_tls_cert),//证书
                             .server_key = mg_str(s_tls_key) };//私钥
  ...
  while (true)
    mg_mgr_poll(&mgr, 100);  // Infinite event loop, blocks for upto 100ms 无限事件循环，最多阻塞100ms
  // unless there is network activity 除非有网络活动
  mg_mgr_free(&mgr);         // Free resources 释放资源
  return 0;
}
